 ...
 */
export function runFullFileRewrite(filePath, initialContent, targetRange, replacementContent) {
 ...
 */
export function runExactStringReplace(filePath, targetNodeContent, replacementContent) {
    ...
    const newContent = currentContent.slice(0, matchIdx) +
        replacementContent +
 ...
 */
export function runUnifiedDiff(filePath, initialContent, targetRange, replacementContent) {
    const before = initialContent.slice(0, targetRange.start);
    const after = initialContent.slice(targetRange.end);
    ...
    for (const hunk of hunks) {
        ...
        for (let offset = 0; offset < currentLines.length; offset++) {
            ...
            if (checkUp >= 0)
                candidatesToCheck.push(checkUp);
            if (checkDown < currentLines.length && checkDown !== checkUp)
                candidatesToCheck.push(checkDown);
 ...
 */
export function runLineHashPatch(filePath, initialContent, targetRange, replacementContent) {
 ...
 */
export function runMDSafeEdit(filePath, initialContent, currentContent, targetRuntimeId, replacementContent, allowedRoots) {
