...
export interface MarkdownNode {
  ...
  rawHash: string;
  content: string; // The sliced node content
  level?: number;  // Depth for headings
  blockId?: string;
  calloutType?: string;
}
 ...
 */
function mapNodeType(astType: string): NodeType {
  switch (astType) {
    case 'root':
      return 'document';
    case 'heading':
 ...
 */
export function parseMarkdownToNodes(bytes: Uint8Array, content: string): MarkdownNode[] {
  ...
  // Recursive walk
  function walk(astNode: any, parentId?: string): string | null {
...
function parseFrontmatterFields(content: string, startCharOffset: number): FrontmatterFieldRange[] {
  if (!content.startsWith('---')) {
    return [];
  }

  const fields: FrontmatterFieldRange[] = [];
